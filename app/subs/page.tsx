...
export default function SubsPage() {
  ...
  const fetchSubsPosts = async () => {
    try {
      setLoading(true)
      const { data, error } = await supabase
        .from("posts")
        .select("*")
        .eq("tag", "Looking for Sub")
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <div className="mb-6">
        <h1 className="text-2xl font-bold">Looking for Sub</h1>
        <p className="mt-2 text-sm text-gray-600">
          Posts looking for substitute players
        </p>
      </div>
      ...
      {!loading && posts.length === 0 && (
        <div className="rounded-lg border bg-white p-8 text-center">
          <p className="text-gray-500">No posts looking for substitutes yet.</p>
        </div>
      )}
